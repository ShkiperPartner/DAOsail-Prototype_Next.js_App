...
## 🆕 Обновления версии 0.5.0 (2025-01-17)

### Новая функциональность:
- **Двухэтапный гостевой режим** с умными лимитами (3+3 вопроса)
- **EmailCapture компонент** (`components/ui/email-capture.tsx`) для сбора лидов
- **Расширенный AppContext** с состояниями `guestStage`, `emailCaptured`, `totalQuestionsAsked`
- **Обновленный ChatBox** с интеграцией форм email capture и регистрации

### Важные изменения:
- **next.config.js**: убран `output: 'export'` для совместимости с API routes
- **AppContext**: новая функция `captureEmail()` для сохранения лидов в Supabase
- **Guest Flow**: мягкие переходы между этапами без резких блокировок

### Новые паттерны:
```typescript
// Использование нового гостевого контекста
const { guestStage, emailCaptured, totalQuestionsAsked, captureEmail } = useAppContext();
...
*Последнее обновление: 2025-01-17 (v0.5.0 Guest Flow Implementation)*